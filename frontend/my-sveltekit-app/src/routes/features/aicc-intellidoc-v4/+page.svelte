...
<script lang="ts">
  ...
  const logger = createTemplateLogger('aicc-intellidoc-v4', 'SelectionPage');
  ...
  async function loadTemplateConfiguration() {
    logger.templateAction('load_configuration', 'aicc-intellidoc-v4');
    
    try {
      loading = true;
      const configResult = await cleanUniversalApi.getTemplateConfiguration('aicc-intellidoc-v4');
      template = configResult.discovery || configResult;
      logger.templateAction('configuration_loaded', 'aicc-intellidoc-v4', template);
    } catch (error) {
  ...
  async function createProject() {
    ...
    logger.templateAction('create_project_start', 'aicc-intellidoc-v4', {
      name: projectName,
    ...
    try {
      ...
      logger.templateAction('project_created', 'aicc-intellidoc-v4', newProject);
